...
- **Reply:** `mcp__github__add_reply_to_pull_request_comment` needs the REST **comment id** (the number in a comment's `html_url`, e.g. `#discussion_r3304458907` becomes `3304458907`).
...
1. **Address and reply first.** Fix the code (or push back), then post one reply per thread with `add_reply_to_pull_request_comment`. Keep any one reply's response `node_id` (a `PRRC_…` string); it carries the repo bytes. Reply before resolving so the note lands before the thread collapses.
...
- For the full gather, investigate, fix, verify, and reply loop, use the `address-pr-review` skill. This skill is only the MCP resolve step.
