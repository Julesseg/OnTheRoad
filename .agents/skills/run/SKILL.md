---
name: run
description: Run the on-the-road Expo app on the iOS simulator (iPhone 17 Pro). Use when asked to run, launch, start, or relaunch the app, or to verify a change in the running app. Use the simulator by default. Only use the physical-device section if the user explicitly asks to run on their iPhone or device.
---
...
Before booting a simulator, record the devices already booted with
`xcrun simctl list devices booted`. When verification ends, shut down every
simulator this run booted and leave pre-existing simulators alone. If none
remain booted, quit the Simulator app.
...
3. **Do not rely on `npx expo run:ios --device`.** On iOS 26 devices its JS-based installer fails at the
   ...
   ```bash
   # Build (this compiles and signs; the final install error is expected)
