...
import { PinInfoCard } from '@/components/pin-info-card.android';
import type { Item } from '@/lib/schema';

// Android (Material surface) variant. Same content/behaviour as the iOS card; the
// glass becomes a Material surface and icons render via the globally-stubbed
// MaterialIcons.
const noop = () => {};

describe('PinInfoCard (Android)', () => {
  it('shows the same info as the itinerary row: type label, name, and lines', () => {
    const item: Item = {
      category: 'meal',
      id: 'lunch',
  ...
  it('routes its body tap to the full item', () => {
    const onOpen = vi.fn();
  ...
  it('offers Directions only when the item has a maps destination', () => {
    ...
    const { rerender } = render(<PinInfoCard item={located} onOpen={noop} onNavigate={onNavigate} />);
