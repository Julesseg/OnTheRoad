...
// Android (Compose) /days variant. Mirrors days-sheet.test.tsx behaviour: the
// expo-router header chrome (Stack.Header / Stack.Title / Stack.Toolbar) is the
// same, so the same toolbar-button DOM stand-ins drive the Day-filter visibility
// asserts. The divergent render tree is the expanded large title, which now uses
// @expo/ui/jetpack-compose (globally aliased to a DOM stub in vitest.config.ts) —
// no swift-ui mocks needed; the Compose Column/Row/Text render as div/div/span.
//
...
const renderDays = async (opts: {
  ...
  const { default: DaysSheet } = await import('@/app/days.android');
  render(<DaysSheet />);
  return state;
};
...
describe('DaysSheet header (Android)', () => {
  ...
  it('renders the expanded large title with the trip name and countdown pill', async () => {
    await renderDays({ trip: inProgressTrip });

    // The Compose Column/Row/Text large title renders the trip name as a <span>.
    expect(screen.getAllByText('Coast Run').length).toBeGreaterThan(0);
  });
});
